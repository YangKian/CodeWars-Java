 ...
 *
 * Remarks
 * a or b might be [] (all languages except R, Shell). a or b might be nil or null or None or nothing (except in
...
public class Are_they_the_same {
    public static boolean comp(int[] a, int[] b) {
        ...
        Arrays.sort(b);
        //直接循环执行b[i] == a[i]*a[i]会通不过测试。。。不懂为什么
        int[] a_square = Arrays.stream(a).map(i -> i * i).sorted().toArray();
        return Arrays.equals(a_square, b);
    }
}
